...
class semiLASER(BaseSpectroscopy):
    def __init__(self, path_or_BrukerExp, *args, **kwargs):
        ...
        if self.NR > 1:
            if self.n_receivers==1:
                self.time_ax_array = np.array([np.arange(n, self.NR * self.TR * self.Nvox, self.TR * self.Nvox) for n in
                                               range(self.Nvox)])  # in seconds
            else:
                self.time_ax_array = self.time_axis
                logger.warning('Caution, self.time_axis might not be correct for dynamic MV-sLASER here')
